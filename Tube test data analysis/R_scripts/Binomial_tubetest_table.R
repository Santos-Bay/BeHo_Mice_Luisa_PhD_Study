...
merged_data <- merged_data[,-c(6,8,9,11)] #remove irrelevant columns
...
#### The NAs are automatically there when reading in the csv files from end of Nov 2023 ###
...
sorted_merged <- merged_dates[order(merged_dates$Name), ]

sorted_merged$TubeTest_ID <- c(rep("T01",20),rep("T02",20),rep("T03",20),rep("T04",20),
...
names(sorted_merged)[1] <- "Trail_ID"
names(sorted_merged)[2] <- "Trial_Number"
names(sorted_merged)[4] <- "Tube_Win_Position"
names(sorted_merged)[6] <- "Win_Loss_Binomial"
...
####### To code the tube test IDs smartly, I started creating this code #######
#Adding a TubeTest_ID column
#sorted_merged <- merged_dates[order(merged_dates$Name), ]
...
#sorted_merged$TubeTest_ID <- new_column
